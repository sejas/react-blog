...
const deleteComment = commentId => fetch(`${URL}/comments/${commentId}`, getOptionsWithMethod("DELETE"))
              .then(res => res.json() || [])
const createComment = (comment) => fetch(`${URL}/comments`, getOptionsWithMethod("POST", {...comment, id:uuid(), timestamp: Date.now()}))
              .then(res => res.json() || [])
const createPost = (post) => fetch(`${URL}/posts`, getOptionsWithMethod("POST", {...post, id:uuid(), timestamp: Date.now()}))
              .then(res => res.json() || [])
